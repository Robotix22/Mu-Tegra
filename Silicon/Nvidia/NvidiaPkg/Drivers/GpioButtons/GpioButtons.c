...
GetButtonState ()
{
  ...
  EFI_DEVICE_PATH_PROTOCOL          *KeypadDxeDevicePath = (EFI_DEVICE_PATH_PROTOCOL *)&KeypadDevicePath;
  EFI_KEY_DATA                       KeyData;
  EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *SimpleEx            = NULL;
  VOID                              *NotifyHandle        = NULL;

  // Locate Device Path of KeypadDxe
  Status = gBS->LocateDevicePath (&gEfiSimpleTextInputExProtocolGuid, &KeypadDxeDevicePath, &Handle);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Failed to Locate Device Path of Keypad! Status = %r\n", Status));
    goto exit;
  }

  // Open the Protocol of KeypadDxe
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Failed to Open Keypad Driver Protocol! Status = %r\n", Status));
    goto exit;
  }
