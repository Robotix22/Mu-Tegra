...
MsBootPolicyLibIsAltBoot (VOID)
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Status    = SettingsAccess->Get (SettingsAccess, DFCI_SETTING_ID__ALT_BOOT, NULL, DFCI_SETTING_TYPE_ENABLE, &ValueSize, &EnableAltBoot, NULL);
    
    if (!EFI_ERROR (Status) && !EnableAltBoot) {
      DEBUG ((EFI_D_WARN, "%a EnableAltBoot is disabled.\n", __FUNCTION__));
      return FALSE;
    }
  }
