...
TEGRA_GPIO_PROTOCOL *mGpioProtocol;
...
KeypadDeviceConstructor (VOID)
{
  ...
  Status = gBS->LocateProtocol(&gTegraGpioProtocolGuid, NULL, (VOID *)&mGpioProtocol);
  if (EFI_ERROR (Status)) {
  ...
  StaticContext->Gpio = TEGRA_GPIO(X, 5);
  ...
  UINT64                  Delta)
{
  ...
  for (Index = 0; Index < (sizeof(KeyList) / sizeof(KeyList[0])); Index++) {
    ...
    IsPressed = !mGpioProtocol->GetState(Context->Gpio);
